...
function predictLstFromIndices(collection, geometry, targetYear, startYear, endYear) {
  const elevation = ee.Image('USGS/SRTMGL1_003').select('elevation').toFloat().rename('elevation');
  ...
  const coefficients = ee.Array(regression.get('coefficients')).project([0]).toList();

  return ee.Image.constant(ee.Number(coefficients.get(0)))
    .add(ndviTarget.multiply(ee.Number(coefficients.get(1))))
    .add(ndbiTarget.multiply(ee.Number(coefficients.get(2))))
...
export async function runGeeEvaluation({
  ...
  // ── Parallel evaluate() ───────────────────────────────────────────────────
  const [mapId, statsRaw, chartRaw, impactRaw, scatterRaw, baselineRaw, sceneRaw] =
    await Promise.all([
