...
export async function runGeeEvaluation({
  ...
  // Seluruh tahun yang disentuh periode, bukan hanya tahun kalender penuh.
  // Tahun yang tercakup sebagian tetap punya citra dan tetap punya rata-rata
  // yang bisa dihitung; yang tidak boleh adalah menyajikannya seolah setara
  // dengan tahun penuh. Karena itu ia digambar, lalu ditandai — lihat `complete`
  // saat pembentukan chartByRegion di bawah.
  const years = ee.List.sequence(seriesStartYear, seriesEndYear);

  // Sebuah tahun disebut lengkap kalau ia berada di dalam rentang tahun kalender
  // penuh — persis keluaran resolveFullYearRange(), jadi definisi "lengkap" di
  // sini satu pintu dengan validator dan tidak bisa menyimpang darinya.
  const isCompleteYear = (year) => hasFullYear && year >= startYear && year <= endYear;
  ...
  //
  // filterDate di sini hanya bisa mempersempit, tidak pernah memperlebar:
  // dualCollection sudah disaring ke tanggal pilihan pengguna di
  // buildDualCollection(). Jadi pada periode tanpa tahun penuh sekalipun, yang
  // dihitung tetap persis rentang yang dipilih — tidak ada citra di luar itu
  // yang ikut tertarik masuk.
  ...
  for (const name of ['ALL', ...REGION_LIST]) {
    statsByRegion[name] = {
      ...
      // Tanpa satu pun tahun kalender penuh, label "2026–2026" akan terbaca
      // sebagai setahun utuh padahal isinya bisa enam bulan. Tanggal aslinya
      // ikut dikirim supaya klien bisa memberi label yang benar, dan flag-nya
      // dipakai untuk memutuskan label mana yang dipakai.
      has_full_year: hasFullYear,
      period_start: startDateInput,
      period_end: endDateInput,
    };
  }
  ...
  for (const feature of chartRaw.features || []) {
    ...
    chartByRegion[props.region].push({
      year: props.year,
      value: props.mean,
      // Titik tahun tercakup sebagian tetap dikirim, tapi klien wajib bisa
      // membedakannya: rata-rata enam bulan yang kebetulan jatuh di musim
      // kemarau tidak sebanding dengan rata-rata dua belas bulan, sekalipun
      // keduanya berdiri di sumbu yang sama.
      complete: isCompleteYear(props.year),
    });
  }
  ...
  // GEE tersendiri: menjumlahkan tanggalnya menurut bulan memberi angka yang
  // sama persis tanpa biaya tambahan.
  //
  // Tahun tak lengkap disaring keluar di sini. scenesByYear sekarang mencakup
  // seluruh tahun yang disentuh periode, sementara profil musiman tetap dihitung
  // dari fullYearCollection. Tanpa penyaringan ini, tooltip musiman akan
  // menyebut jumlah citra dari tahun yang nilainya sendiri tidak ikut dihitung.
  const sceneCountByMonth = new Array(12).fill(0);
  for (const item of scenesByYear) {
    if (!isCompleteYear(item.year) && hasFullYear) continue;
