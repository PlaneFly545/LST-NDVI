...
export function parseQueryParams(query) {
  ...
  // Seluruh agregasi tahunan dibatasi tahun kalender penuh.
  const fullYears = resolveFullYearRange(startDateObj, endDateObj);
  if (!fullYears) {
    return {
      error: 'Periode harus memuat minimal satu tahun kalender penuh (1 Januari–31 Desember).',
      status: 400,
    };
  }
  const { startYear: baselineStartYear, endYear: baselineEndYear } = fullYears;
